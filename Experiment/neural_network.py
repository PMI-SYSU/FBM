...
class R_FBMP3_Net(nn.Module):
    def __init__(self, device, activator, in_features=784, hidden1_features=100,
        ...
        self.fc_W1 = torch.nn.Parameter(
            torch.randn(in_features, hidden1_features) / torch.sqrt(torch.tensor(in_features))).to(device)
        self.fc_W2 = torch.nn.Parameter(
            torch.randn(hidden1_features, out_features) / torch.sqrt(torch.tensor(hidden1_features))).to(device)
