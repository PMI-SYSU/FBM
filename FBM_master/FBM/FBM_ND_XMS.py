...
batch_size = 50
Num_data= [1000,2000,3000,4000,5000,6000]#[1000,2000,3000,4000,5000,6000,7000,8000,9000,10000]
...
for num_dataset in (Num_data):
        for fermi_surface in fermi_surface_list:
            for run in (runs):
                ...
                np.savetxt(f"data/acc/NUM_tra_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", final_acc_tra_row)
                np.savetxt(f"data/acc/NUM_test_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", final_acc_test_row)
                np.savetxt(f"data/loss/NUM_tra_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", final_loss_tra_row)
                np.savetxt(f"data/loss/NUM_test_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", final_loss_test_row)
                np.savetxt(f"data/distance/NUM_loss_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", loss_tra_row)
                np.savetxt(f"data/distance/NUM_distance_bose_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", distance_bose_row_mini)
                np.savetxt(f"data/distance/NUM_distance_fermi_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv", distance_fermi_row_mini)
