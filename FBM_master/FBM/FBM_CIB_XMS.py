...
batch_size = 50
Num_data= [200]
ratio_list = [0.56]#[0.38,0.4,0.42,0.44,0.46,0.48,0.5,0.52,0.54,0.56,0.58,0.6,0.62]#[0.35,0.4,0.45,0.5,0.55,0.6,0.65]#
fermi_surface_list = [0.4]#[0.4,0.5,0.6]
    ...
    for num_dataset in (Num_data):
        for fermi_surface in fermi_surface_list:
            for run in runs:
                ...
                # 每次储存的网络参数
                upload_weight = f"data/weight/CIB_dF={fermi_surface}_lam={lam}_{run}.pth"
                ...
                #np.savetxt(f"data/count/CIB_tra_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", class_counts_array)
                #np.savetxt(f"data/acc/CIB_tra_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", final_acc_tra_row)
                #np.savetxt(f"data/acc/CIB_test_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", final_acc_test_row)
                #np.savetxt(f"data/loss/CIB_tra_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", final_loss_tra_row)
                #np.savetxt(f"data/loss/CIB_test_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", final_loss_test_row)
                #np.savetxt(f"data/distance/CIB_loss_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", loss_tra_row)
                #np.savetxt(f"data/distance/CIB_distance_bose_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", distance_bose_row_mini)
                #np.savetxt(f"data/distance/CIB_distance_fermi_dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run}.csv", distance_fermi_row_mini)
                logging.info(f" data dF={fermi_surface}_lam={lam}_num={num_dataset}_batchsize={batch_size}_0={round(class_ratio[0],2)}_1={round(class_ratio[1],2)}_{run} has saved")

#%%

#0.38 : 0.48081632653061224

#0.5: 0.5102040816326531

#0.62: 0.48081632653061224

#0.44: 0.5028571428571429