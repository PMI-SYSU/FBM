...
for fermi_surface in fermi_surface_list:
    ...
    for num_dataset in Num_data:
        ...
        for run in runs:
            ...
            acc_path = f"data/acc/FM_test_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv"
            fermi_path = f"data/distance/FM_distance_fermi_dF={fermi_surface}_lam={lam}_num={num_dataset}_{run}.csv"
...
plt.savefig('./result/ND_acc_FM.pdf',bbox_inches='tight')
#plt.legend()
plt.show()
...
for i in range (len(fermi_surface_list)):
    ...
    plt.axhline(y=fermi_surface_list[i], linestyle= '--', color=Color_list[i], linewidth = 2.5, alpha= 0.7)
plt.ylabel('Fermion-pair distance')
plt.xlabel('Training data size')
plt.tight_layout()

plt.savefig('./result/ND_FD_FM.pdf', bbox_inches='tight')
plt.show() 
